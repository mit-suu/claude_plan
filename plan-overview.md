...
Cập nhật 2026-09-16: T17 + T18 đã code xong trên hai nhánh riêng, **chưa merge vào `develop`** (chờ phương án merge/push). Hai nhánh cùng gốc `323e6b4`, không chạm file chung nào ngoài `docs/spec-gaps.md` (mỗi bên chỉ **thêm dòng** ở cuối — conflict nếu có là dạng append, gộp cả hai khối). Việc treo chung của W4: lượt chạy provider thật cho T18 (khung `E2E_AI=1` không dùng được — xem spec-gaps), endpoint `POST /baseline` của T19 để đóng nốt DoD 4 của T17. T17 đã kiểm xong trên BE + Mongo + GLM thật (project run20, đã undo lại; 3 credit) — chi tiết ở `report-t17-change-flow.md`.
