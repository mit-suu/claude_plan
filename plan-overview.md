...
Cập nhật 2026-09-16: T17 + T18 đã code xong trên hai nhánh riêng, **chưa merge vào `develop`** (chờ phương án merge/push). Hai nhánh cùng gốc `323e6b4`, không chạm file chung nào ngoài `docs/spec-gaps.md` (mỗi bên chỉ **thêm dòng** ở cuối — conflict nếu có là dạng append, gộp cả hai khối). Việc treo chung của W4: **`E2E_AI=1`** cho T18 (thiếu API key), endpoint `POST /baseline` của T19 để đóng nốt DoD 4 của T17.
