...
| 4 | T20 | D | Đang làm | 2026-09-16 | DoD 4/4 trên mock (nhánh `feat/FLF-160-discovery-brief`, BE + FE, chưa merge). 13 step Brief + 4 step S-1 qua step runner, ghi Spine bằng op; bỏ `CHAT_DISCOVERY`; FE hết suy state từ tin nhắn, thêm 3 panel Brief. BE 667 test xanh, FE 192 test xanh + lint sạch. **Còn: lượt chạy thật trên dev** |
| — | **M4** | cả 4 | [ ] | | |
| 5 | T21 | A | Chưa làm | | |
| 5 | T22 | B | Chưa làm | | |
| 5 | T23 | D | Chưa làm | | |
...
Bốn XREQ đã **granted 2026-09-16**: T19→T03 và T20→T03 (`prompt-assets.test.ts`), T20→T11 (2 dòng
`STEP_SKILLS`), T19→T16 (`lib/api/export.ts` gửi `base_version` — commit riêng trên nhánh FE
`feat/FLF-159-s9-baseline`). Bốn nhánh đã push: BE và FE mỗi repo hai nhánh.
