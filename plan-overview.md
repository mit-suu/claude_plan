...
<<<<<<< HEAD
| 5 | T21 | A | Chưa làm | | |
| 5 | T22 | B | Đang làm | 2026-09-16 | DoD 0/3 tick đủ. Nhánh BE `feat/FLF-162-integration-tests` (`9a072ff`, đã push). Vitest 3 project `unit`/`integration`/`e2e-ai`; Mongo in-memory replica set + supertest; 10 file integration (57 test), mock ở tầng `callLLM` nên credit/`Usage` là code thật. BE **759 test xanh / 26 skip**, coverage `spine` 95.91% / `pipeline` 94.01% (ngưỡng 70%). `measure:tokens` estimate 19 màn: 271 credit (ngoại suy 20 vòng S-5: 411), input 1.26× — không bậc hai. **Còn: `E2E_AI=1` op cases + `--mode real` (hoãn có chủ ý), CI GitHub, ngưỡng nhóm chưa đặt.** Báo cáo: `report-t22-integration-tests.md` |
| 5 | T23 | D | Chưa làm | | |
=======
...
>>>>>>> ffe9f07fa666c6a50c324049da666600e5aaacb6
| 5 | T24 | C | Chưa làm | | |
| — | **M5** | cả 4 | [ ] | | |

**Tiến độ wave:** W1 7/7 · W2 5/5 · W3 4/4 · **W4 4/4 (code xong, đã merge `develop`)** · W5 3/4 đã merge (T21, T22, T23; còn T24) · **Tổng 23/24** · Merge point: **M1 [x]** · M2 [ ] · **M3 [x]** · **M4 [ ] hoãn có chủ ý** · M5 [ ].
